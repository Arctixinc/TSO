...
async def fix_metadata_command(client: Client, message: Message):
    ...
    start_btn = InlineKeyboardMarkup([
        [InlineKeyboardButton("🏁 Start Fix", callback_data="start_fix_menu")]
    ])
    await message.reply_text("Metadata Fixer System", reply_markup=start_btn)
...
async def start_fix_menu_callback(client: Client, callback_query: CallbackQuery):
    menu_btns = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔄 All Fix", callback_data="fix_all")]
    ])
    await callback_query.message.edit_text("Select what to fix:", reply_markup=menu_btns)
...
async def fix_action_callback(client: Client, callback_query: CallbackQuery):
    ...
    if mode == "movies":
        status_text = "🔄 Initializing Movie Fix..."
    elif mode == "tv":
        status_text = "🔄 Initializing TV Show Fix..."
    else:
        status_text = "🔄 Initializing Full Library Fix..."

    await callback_query.message.edit_text(status_text, reply_markup=cancel_btn)

    try:
        fix_task = asyncio.create_task(run_fix_process(client, callback_query.message, mode))
    except Exception as e:
        is_fixing = False
        await callback_query.message.edit_text(f"❌ Error starting fix process: {e}")
...
async def run_fix_process(client: Client, status_msg: Message, mode: str):
    ...
    try:
        ...
        async def update_status():
            ...
            text_header = "🔄 **Fixing Metadata**"
            if mode == "movies": text_header += ": Movies"
            elif mode == "tv": text_header += ": TV Shows"
            elif mode == "all": text_header += ": All Fix"

            text = (
                f"{text_header}\n\n"
                f"**Progress:** [{bar}] {percent:.1f}%\n"
                f"✅ **Completed:** {stats['processed']}/{total_items_to_process}\n"
                f"⚠️ **Skipped:** {stats['skipped']}\n"
                f"❌ **Errors:** {stats['errors']}\n\n"
                f"⏱ **Elapsed:** {format_time(elapsed)}\n"
                f"⏳ **ETA:** {format_time(eta_seconds)}\n\n"
                f"{stats['current_name']}"
            )
        ...
        # Final Summary
        summary_text = (
            f"✅ **Fix Completed: {mode.title()}**\n\n"
            f"📊 **Total Items:** {total_items_to_process}\n"
        ...
        if should_cancel:
            summary_text = "🛑 **Operation Cancelled**\n\n" + summary_text
