...
def build_main_markup(index: int, total: int, url: str, view_mode: str):
    ...
    info_row = [
        ...
        InlineKeyboardButton(f"Mode: {'Tail' if view_mode == 'tail' else 'Head'}", callback_data="log_toggle_view_mode")
    ]
...
def build_selector_markup(msg_id: int):
    ...
    buttons = []
    buttons.append([InlineKeyboardButton("Select Page", callback_data="selector_null")])
...
# -------------------------------
@Client.on_callback_query(filters.regex("^selector_(prev|next|first|last)$"))
